...
-- This is enough?
-- Somehow this passed all the precedence tests, that ware generated by Gemini 3 Fast
	...
	if t.T == "i" then
		...
		elseif t.i == "function" then
			...
			if not (k.T == "p" and k.i == "(") then
				error("Function definition expects the parameters to begin with `(`")
			end
			repeat
				C[2] = C[2] + 1
				tok_trim(C)
				...
				elseif k.T == "i" then
					...
					error("Expecting an ident for the function parameters")
				end
				if k.T == "p" and k.i == ")" then
...
ast_parse_expr = function(C)
	...
	if t.T == "i" then
		if t.i == "and" then
			...
			local r = ast_parse_expr(C)
			if not r then
				error("Invalid `and` expression no, right at: " .. t.span[1] .. "-")
			end
		...
		elseif t.i == "or" then
			...
			local r = ast_parse_expr(C)
			if not r then
				error("Invalid `or` expression no, right at: " .. t.span[1] .. "-")
			end
	...
	elseif t.T == "p" then
		...
		if t.i == "+" then
			if not r then
				error("Invalid `+` expression no, right at: " .. t.span[1] .. "-")
			end
	...
	repeat
		...
		repeat
			...
			if a.T == "p" and a.i == "[" then
				...
				l = ast_parse_expr(C)
				if not l then
					error("Invalid expression in table key, at: " .. C[2])
				end
				a = C[1][C[2]]
				if not a or a.T ~= "p" or a.i ~= "]" then
...
local parse_function = function(C, is_local)
	...
	if is_local then
		...
		if keywords[t.i] then
			error("keyword used as the function name for `local function`: " .. t.i)
		end
	...
	if t.T == "i" then
				...
				repeat
					...
					if a.T == "i" then
						...
						elseif a.i == "elseif" then
							...
							l = ast_parse_expr(C)
							if not l then
								error("Expect expression after elseif")
							end
							table.insert(c, l)
							tok_trim(C)
							a = C[1][C[2]]
		...
		elseif t.i == "for" then
			...
			if a.i == "=" then
				...
				if not s then
					error("for, expect initial value of " .. n.i .. "as expression")
				end
				tok_trim(C)
				a = C[1][C[2]]
				if not a or a.T ~= "p" then
			...
			while a and (a.T == "p" and a.i == ",") do
				...
				if keywords[a.i] then
					error("reserved keyword used as parameter name in for")
				end
				table.insert(k, a.i)
				C[2] = C[2] + 1
				tok_trim(C)
